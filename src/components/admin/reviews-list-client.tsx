...
import {
  ...
  restoreReview as restoreReviewAction,
} from "@/app/admin/(panel)/reviews/actions"
import { useRouter } from "next/navigation"

type StatusFilter = ReviewStatus | "all"

export interface ReviewsListClientProps {
  initialReviews: MockReview[]
}

export function ReviewsListClient({ initialReviews }: ReviewsListClientProps) {
  const router = useRouter()
  const [reviews, setReviews] = React.useState<MockReview[]>(initialReviews)
  const [search, setSearch] = React.useState("")
  const [statusFilter, setStatusFilter] = React.useState<StatusFilter>("pending")

  // Re-sync when server data changes (after revalidatePath)
  React.useEffect(() => {
  ...
  const approve = async (id: string) => {
    ...
    setReviews((rs) =>
      rs.map((r) =>
        r.id === id
          ? { ...r, status: "approved" as ReviewStatus, approvedAt: new Date().toISOString() }
          : r
      )
    )
