...
export default function OrdersPage() {
  ...
  const stats = React.useMemo(() => {
    ...
    const verifyPayment = mockOrders.filter(
      (o) => o.status === "payment_uploaded"
    ).length
    const preparing = mockOrders.filter((o) =>
      ["paid", "preparing"].includes(o.status)
    ).length
    const inTransit = mockOrders.filter((o) => o.status === "shipped").length
    const total = mockOrders.length
    return { pending, verifyPayment, preparing, inTransit, total }
  }, [])
      ...
      {filtered.length > 0 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>
            ...
            de{" "}
            <span className="tabular-nums">{mockOrders.length}</span> pedidos
          </span>
        </div>
      )}
    </div>
